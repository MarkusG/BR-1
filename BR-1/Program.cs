...
namespace BR_1
{
    class Program
    {
        ...
        public void CreateCommands()
        {
            ...
            cService.CreateCommand("faction")
                ...
                .Do(async (e) =>
                {
                    ...
                    {
                        case "vs":
                            await e.User.RemoveRoles(_planetside.GetRole(terranID));
                            await e.User.RemoveRoles(_planetside.GetRole(conglomerateID));
                            await e.User.AddRoles(_planetside.GetRole(vanuID));
                            break;
                        case "tr":
                            await e.User.RemoveRoles(_planetside.GetRole(vanuID));
                            await e.User.RemoveRoles(_planetside.GetRole(conglomerateID));
                            await e.User.AddRoles(_planetside.GetRole(terranID));
                            break;
                        case "nc":
                            await e.User.RemoveRoles(_planetside.GetRole(vanuID));
                            await e.User.RemoveRoles(_planetside.GetRole(terranID));
                            await e.User.AddRoles(_planetside.GetRole(conglomerateID));
                            break;
                        case "remove":
                            await e.User.RemoveRoles(_planetside.GetRole(vanuID));
                            await e.User.RemoveRoles(_planetside.GetRole(terranID));
            ...
            cService.CreateCommand("v6")
                ...
                .Do(async (e) =>
                {
                    if (e.User.HasRole(_planetside.GetRole(vanuID)))
                    {
                    ...
                    else if (e.User.HasRole(_planetside.GetRole(terranID)))
                    {
                    ...
                    else if (e.User.HasRole(_planetside.GetRole(conglomerateID)))
                    {
            ...
            cService.CreateCommand("getIDs")
                ...
                .Do(async (e) =>
                {
                    ...
                    {
                        foreach (Role r in _planetside.Roles)
                        {
                            Console.WriteLine($"{r.Name} : {r.Id}");
                        }
                        foreach (Channel c in _planetside.TextChannels)
                        {
            ...
            cService.CreateCommand("pp")
                .Do(async (e) =>
                {
                    await _client.CurrentUser.Edit(username: "BR_1");
                });
            #endregion
        }
        ...
        public void CreateOnServerAvailable() // Define _planetside as the correct server. For bots with only one server, you can let your server variable equal e.Server
        {
            _client.ServerAvailable += (s, e) => { _planetside = _client.GetServer(166320083000492032); };
        }
